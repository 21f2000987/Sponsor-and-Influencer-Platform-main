...
class User(UserMixin, datab.Model):
    ...
    role = datab.Column(datab.String(50), nullable=False, unique=True)
    ...
    def __repr__(self):
        return f"<User id={self.id}, username='{self.username}', role='{self.role}', reach='{self.reach}', niche='{self.niche}', name='{self.name}'>"
...
class AdRequest(datab.Model):
    ...
    username = datab.Column(datab.String(150), datab.ForeignKey('users.username'), nullable=False)
